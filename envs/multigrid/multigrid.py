...
class Floor(minigrid.Floor):
  """
  Patch for minigrid.Floor.render which is broken in gym_minigrid==1.0.1
  """

  def render(self, img):
    # This new render function is only necessary because the default floor one is
    # broken in gym_minigrid==1.0.1
    # (this function is taken from a newer version of minigrid)

    # Give the floor a pale color
    color = minigrid.COLORS[self.color] / 2
...
class MultiGridEnv(minigrid.MiniGridEnv):
  ...
  def __init__(
    ...
    self.tile_current_pos = [None] * self.n_agents
  ...
  def move_agent(self, agent_id, new_pos):
    ...
    assert not (old_pos == new_pos).all()
    ...
    self.grid.set(old_pos[0], old_pos[1], self.tile_current_pos[agent_id])
    # store object at upcoming tile
    # Note: if we want to "collect" objects, simply set self.tile_current_pos[agent_id] = None
    self.tile_current_pos[agent_id] = self.grid.get(*new_pos)
