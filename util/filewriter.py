...
# 
# More robust logging when restarting run after preemption or crash by Samuel Garcin.
...
class FileWriter:
    def __init__(
        ...
        if os.path.exists(self.paths["meta"]):
            ...
            with open(self.paths["meta"], "r") as f:
                ...
                for arg in old_meta["args"]:
                    if arg not in ["log_dir", "verbose", "dataset_path", "generative_model_path"]:
        ...
        self.final_test_eval_fieldnames = ['num_test_seeds', 'mean_episode_return', 'mean_solved_rate']
        self.level_seeds_fieldnames = ['n_update', 'acceptance_rate', 'n_proposed', 'n_accepted', 'new_seeds',
                                       'new_seed_indices']
        ...
        if os.path.getsize(self.paths["level_seeds"]) == 0:
            self._levelseedswriter.writeheader()

        self._finaltestwriter.writeheader()

    def check_log_headers(self, to_log: Dict):
        """
        This function is called before calling .log() for the first time.
        It checks if an existing log file is present, if it is, it reads the contents and checks that the headers
        match the stats. If some headers are missing or not in the sames orders as in the variable to_log we rewrite the
        logfile using the keys from to_log as the new headers.
        """
        ...
        with open(self.paths["logs"], "r") as f:
            ...
            if old_headers is None:
                return
            elif old_headers == list(to_log.keys()):
                return
            self._logger.warning(
                "Log file headers do not match the stats to log. Will update the log file."
            )
            data = list(reader)
        ...
        with open(self.paths["logs"], "w") as f:
            ...
            for row in data:
                row["_tick"] = row.pop("# _tick")
                writer.writerow(row)

        # We need to reopen the file to make sure the writer is in sync with the file.
        self._logfile = open(self.paths["logs"], "a")
        self._logwriter = csv.DictWriter(self._logfile, fieldnames=self.fieldnames)
    ...
    def log_level_weights(self, weights, n_update, seeds=None, n_proposed=0, n_accepted=0):
